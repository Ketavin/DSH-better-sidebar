...
侧边栏内容、文件预览与常规设置的开关统一使用 DSH 主题强调色；卡片采用淡色高亮，随浅色、深色及跟随系统外观同步变化。

- **🗂️ 文件工作台**：资源管理器（懒加载目录树；软链接按目标类型展示——目录软链接可展开、失效链接标红）+ CodeMirror 编辑器；图片（适应窗口 / 100% / 25%–400% 缩放）/ Markdown（含 Mermaid 图表，strict 安全渲染 + 点击放大；README 级内嵌 HTML——徽章墙 / `<details>` 折叠 / 表格内联标签经 DOMPurify 消毒真实渲染；浮动目录大纲一键跳转）/ HTML / PDF。鼠标位于图片、Markdown 或代码查看器内时，`Ctrl/Cmd + 滚轮`只缩放当前 Sidebar 内容，不触发浏览器整页缩放
...
| **🖥️ CodeMirror 代码编辑器**<br/><div align="center"><img width="420" alt="CodeMirror 代码编辑器" src="https://github.com/user-attachments/assets/b44b488e-568c-4ee0-b96c-e9c906598a77" /></div> | **🖼️ 图片内联预览**<br/><sub>默认适应当前窗口，也可一键切换 100%，并在 25%–400% 间缩放；大图保留双向滚动查看。</sub><br/><div align="center"><img width="420" alt="图片内联预览" src="https://github.com/user-attachments/assets/f9a58c30-5b7a-48b5-9e22-37d7e071f593" /></div> |
